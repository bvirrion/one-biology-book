...
\cref{ch:b1:biosyntheses-integration}). A \emph{saturated}\index{saturated
fatty acid} fatty acid has only single bonds and a straight, flexible
...
The \omterm{def:b1:lipids:fattyacid}{lipids} of membranes are \emph{amphiphilic}\index{amphiphilic
molecule}: a polar \emph{head} that water solvates and one or two
...
A \omterm{def:b1:membranes-transport:membrane}{lipid bilayer} has a \emph{phase transition temperature}\index{phase
transition temperature} $T_m$: below it the chains are packed in an
...
\qty{8}{MJ/day}, and the mass of hydrated glycogen that would hold the
same energy.
\end{exercise}
...
\qty{60}{MJ/day}. Energy: \omterm{def:b1:lipids:triglyceride}{fat} \qty{38}{kJ/g}, glycogen \qty{17}{kJ/g}
